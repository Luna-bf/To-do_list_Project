...
const elements = {
    ...
    completedTasksTitle: document.querySelector("#completed-tasks-h2"),
    allTasksTitle: document.querySelector("#all-tasks-h2"),
    emptyArrayText: document.querySelector("#empty-array-text"),
};
    ...
    for(let myTask of myTasks) { //J'utilise une boucle for...of
        ...
        */
        ...
        trashIcon.addEventListener('click', () => {
            li.remove(); //Si elle est cliquée, la tâche qui lui est associée est supprimée puis envoyée dans le tableau completedTasks
        });
    }
}
...
elements.form.addEventListener('submit', (e) => {
    ...
    const newTask = {
        title: formData.get('ajouter'), //Je récupère la donnée name (ajouter) de l'input se trouvant dans le form afin d'afficher le titre/nom de la tâche
...
function showParagraphs() {
    if(myTasks.length > 0) {
        elements.emptyArrayText.setAttribute("hidden"); 
    }
}
    // : ,
    // allTasksTitle: document.querySelector("#all-tasks-h2"),
